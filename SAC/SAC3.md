...
3) **Выполнять план пошагово**, повторяя цикл:
   ...
   - обновить working state (progress, facts, missing)
...
- **missing**  
  Что еще нужно получить для корректного завершения.

Формат `facts` и `missing` агент выбирает сам.
...
- При достижении лимита агент:
  ...
  - сообщает текущий progress и missing
...
- Любое `eid` допустимо только как часть текущего `planned_action` и не переносится в facts/missing/plan.
